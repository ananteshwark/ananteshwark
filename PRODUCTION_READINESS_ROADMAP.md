...
| P0.8 | **Backup/DR & data lifecycle runbooks absent.** Nothing defines RPO/RTO, PITR configuration, tenant export, or archival. | Contractual table stakes for enterprise SaaS. | ✅ **BUILT** — `OPERATIONS_RUNBOOK.md`: RPO/RTO targets with PITR + nightly dump procedure and restore drill, zero-downtime deploy/rollback policy (additive migrations), incident playbooks keyed to X-Request-ID and /metrics, security ops (MFA recovery, lockout, rotation), data-retention defaults, quarterly checklist. Tenant export now BUILT: `GET /admin/tenants/:id/export` (super admin) walks entity metadata for every tenant-scoped table and extracts that tenant's rows (row-capped with truncation flags, resilient to pre-migration tables), plus an `/export/summary` row-count preview. |
...
| P1.5 | **Interactive dashboards / embedded analytics.** Analytics module exists (cross-module KPIs); peers ship pivot/drill semantic layer. | SAC embedded; Fusion OTBI | ✅ **BUILT** — code-defined dataset registry (expenses, sales orders, AR invoices, POs, employees, tickets) whitelisting dimensions/measures/date columns; ad-hoc queries compile to parameterized SQL (identifiers only from the registry — zero injection surface, values always bound), month time-grain, tenant scoping enforced in every plan; saved queries with chart-type hints, validated at save time, runnable by id (`/analytics/queries/*`). |
| P1.6 | **Workforce scheduling / shift roster optimization.** Shifts exist; auto-rostering and coverage planning do not. | DarwinBox WFM; SAP TM | ✅ **BUILT** — coverage demand (shift × date × headcount, optional org unit), greedy auto-assigner honoring no-double-booking, approved leave, and weekly shift caps with least-loaded-first fairness; unfillable slots reported, never dropped; DRAFT→PUBLISHED roster lifecycle emitting `roster.published`; manual assign/unassign with the same guards; coverage gap report. |
| P1.7 | **Payroll country packs beyond India.** Statutory engine is India-first; US/UAE localization packs exist for finance, not payroll. | DarwinBox multi-country payroll | ✅ **BUILT** — `Employee.payrollCountry` selects the statutory calculator: payroll runs dispatch non-IN employees through the LocalizationRegistry (US pack: federal brackets + FICA; AE pack: GPSSA + gratuity) while India keeps the config-driven PF/ESI/PT/TDS path; zero-amount lines dropped; one run mixes countries safely. |
...
- **Continuous close / anomaly detection on journals** (S/4 "continuous accounting"). ✅ **BUILT** — `GET /finance/close/anomalies?from&to` screens every posted journal in the window: duplicate reference+amount (HIGH), per-account 3σ amount outliers (HIGH), large round manual totals (MEDIUM), rarely-used accounts (MEDIUM), weekend manual postings (LOW); severity-ranked with per-check summary. **Extended to all modules** — `modules/ai` anomaly layer: shared scoring engine (per-group z-score outliers, exact-duplicate clustering, volume-spike detection) applied to expenses (claim outliers, duplicate claims), procurement (PO outliers, same-day PO clusters, duplicate vendor-invoice refs), sales (order outliers per customer), finance (AR invoice outliers + the journal detector folded in), payroll (net-pay month-over-month swings), CRM (ticket volume spikes). Unified `GET /ai/anomalies` with per-module summary + coverage report and an AI Anomalies page under Analytics; detectors are isolated so one failure never breaks the scan. **Operationalized** — `POST /ai/anomalies/scan-async` queues the scan on the durable jobs queue (any instance picks it up, retries with backoff), findings emit the `anomaly.detected` automation event (tenants attach notify/email/webhook rules instead of polling the page), and the copilot answers "Any anomalies this week?" / "show anomalies in finance" through the same service.
