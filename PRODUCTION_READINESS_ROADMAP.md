...
| P0.5 | **Scheduler durability.** Automation sweeps use in-process `setInterval`; multi-instance deployments would double-fire, and restarts lose schedule state. | Peers use durable job queues (Redis/DB-backed) with leader election. | ◻ Next: DB-lease leader election on the existing scheduler (no new infra), then a `jobs` table for durable one-shot work. |
| P0.6 | **No idempotency keys on mutating APIs.** Retried POSTs (mobile/webhook callers) can double-create documents. | Fusion REST and S/4 OData both support idempotent replay semantics. | ◻ Next: `Idempotency-Key` header + response-replay table on document-creating endpoints. |
| P0.7 | **No optimistic concurrency.** No `@VersionColumn` anywhere; two users editing the same PO silently last-write-wins. | S/4 uses ETags; Fusion uses object version numbers. | ◻ Next: version column + `If-Match` support on high-contention documents (PO, SO, invoice, employee). |
...
|---|---|---|---|
| P1.1 | **E-invoicing / statutory tax returns.** Payroll statutory (India) exists, but there is no GST e-invoice (IRN/QR), no GSTR-1/3B return prep, no PEPPOL/UBL export. | S/4 Document & Reporting Compliance; Fusion tax reporting | `compliance/` module: e-invoice payload builder + IRP adapter interface, return-period aggregation from AR/AP invoices |
| P1.2 | **Bank connectivity formats.** Payment runs exist; no ISO 20022 pain.001 export / camt.053 import (bank files page is India-format oriented). | S/4 Advanced Payment Management | Format writers on the existing payment-run module |
| P1.3 | **Landed cost.** GRN/valuation exist; no freight/duty allocation to inventory cost. | S/4 landed costs; Fusion Cost Mgmt | Cost-adjustment doc allocating charges over receipt lines |
| P1.4 | **Approval matrix depth.** Workflow engine supports role/user approvers; peers ship amount-band × org-unit approval matrices with delegation windows (delegation module exists — matrices don't). | Fusion AMX; S/4 flexible workflow | Rule table (docType, amount band, org unit → approver chain) resolved by the existing engine |
