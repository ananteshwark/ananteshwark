...
export function renderInvoicePrint(data: InvoicePrintData): string {
  const { invoice, customer, lines, companyName } = data;
  const rows = lines
    .map(
      (l, i) => `
      <tr>
...
<style>
  * { box-sizing: border-box; }
  ...
  .doc-title h1 { margin: 0; font-size: 26px; color: #059669; letter-spacing: 1px; }
  ...
  .status { display: inline-block; padding: 2px 10px; border-radius: 999px; font-size: 12px; font-weight: 600; background: #ecfdf5; color: #059669; }
  .print-btn { position: fixed; top: 16px; right: 16px; background: #059669; color: #fff; border: none; padding: 10px 18px; border-radius: 6px; font-size: 14px; cursor: pointer; }
...
<body>
  ...
  <div class="doc">
    <div class="header">
      ...
      <div class="doc-title">
        <h1>INVOICE</h1>
        <div class="meta">No. ${esc(invoice.invoiceNumber)}</div>
        <div class="meta">Date: ${esc(invoice.invoiceDate)}</div>
        <div class="meta">Due: ${esc(invoice.dueDate)}</div>
        <div class="meta"><span class="status">${esc(invoice.status)}</span></div>
      </div>
    </div>
    ...
    <div class="parties">
      <div class="party">
        ...
        ${customer?.code ? `<p>Code: ${esc(customer.code)}</p>` : ''}
        ${customer?.email ? `<p>${esc(customer.email)}</p>` : ''}
        ${customer?.phone ? `<p>${esc(customer.phone)}</p>` : ''}
        ${customer?.taxId ? `<p>Tax ID: ${esc(customer.taxId)}</p>` : ''}
      </div>
      <div class="party" style="text-align:right;">
        <h3>Reference</h3>
        <p>${esc(invoice.reference) || '&mdash;'}</p>
      </div>
    </div>
    ...
    <table class="totals">
      <tr><td class="label">Subtotal</td><td class="num">${money(invoice.subtotal, invoice.currency)}</td></tr>
      <tr><td class="label">Tax</td><td class="num">${money(invoice.taxAmount, invoice.currency)}</td></tr>
      <tr class="grand"><td>Total</td><td class="num">${money(invoice.total, invoice.currency)}</td></tr>
      <tr><td class="label">Amount Paid</td><td class="num">${money(invoice.amountPaid, invoice.currency)}</td></tr>
      <tr><td class="label">Balance Due</td><td class="num">${money(invoice.balanceDue, invoice.currency)}</td></tr>
    </table>

    ${invoice.notes ? `<div style="font-size:13px;color:#6b7280;"><strong>Notes:</strong> ${esc(invoice.notes)}</div>` : ''}
