...
function SodTab() {
  ...
  const [form, setForm] = useState({ name: '', permissionA: '', permissionB: '', severity: 'CRITICAL' });
  const [scan, setScan] = useState<any>(null);
  useEffect(() => { load(); }, []);
  async function load() { setRules(unwrap(await grcApi.listSodRules())); }
  async function create(e: React.FormEvent) { e.preventDefault(); try { await grcApi.createSodRule(form); setForm({ name: '', permissionA: '', permissionB: '', severity: 'CRITICAL' }); load(); } catch (err: any) { alert(err.response?.data?.message ?? 'Failed'); } }
  ...
  return (
    <div className="space-y-3">
      ...
      <table className="w-full text-sm border rounded-lg overflow-hidden">
        <thead className="bg-gray-50"><tr><th className="px-3 py-2 text-left">Rule</th><th className="px-3 py-2 text-left">Conflict</th><th className="px-3 py-2 text-left">Severity</th></tr></thead>
        <tbody className="divide-y">
          {rules.length === 0 ? <tr><td colSpan={3} className="px-3 py-6 text-center text-gray-400">No SOD rules.</td></tr> : rules.map((r) => (
            <tr key={r.id}><td className="px-3 py-2">{r.name}</td><td className="px-3 py-2 font-mono text-xs">{r.permissionA} ✕ {r.permissionB}</td><td className="px-3 py-2"><span className={`text-xs px-1.5 py-0.5 rounded ${SEV[r.severity]}`}>{r.severity}</span></td></tr>
          ))}
        </tbody>
      </table>
    </div>
...
function ControlsTab() {
  ...
  const [form, setForm] = useState({ code: '', name: '', objective: '', testFrequency: 'QUARTERLY' });
  useEffect(() => { load(); }, []);
  async function load() { setControls(unwrap(await grcApi.listControls())); }
  async function create(e: React.FormEvent) { e.preventDefault(); try { await grcApi.createControl(form); setForm({ code: '', name: '', objective: '', testFrequency: 'QUARTERLY' }); load(); } catch (err: any) { alert(err.response?.data?.message ?? 'Failed'); } }
  ...
  return (
    <div className="space-y-3">
      <form onSubmit={create} className="grid grid-cols-4 gap-2 bg-gray-50 p-3 rounded-lg items-end">
        ...
        <button type="submit" className="bg-indigo-600 text-white px-2 py-1 rounded text-sm">+ Control</button>
      </form>
      <table className="w-full text-sm border rounded-lg overflow-hidden">
        ...
        <tbody className="divide-y">
          {controls.length === 0 ? <tr><td colSpan={5} className="px-3 py-6 text-center text-gray-400">No controls.</td></tr> : controls.map((c) => (
            <tr key={c.id}>
              ...
              <td className="px-3 py-2 text-right whitespace-nowrap"><button onClick={() => test(c.id, 'EFFECTIVE')} className="text-green-600 text-xs hover:underline mr-2">effective</button><button onClick={() => test(c.id, 'DEFICIENT')} className="text-red-500 text-xs hover:underline">deficient</button></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function RiskTab() {
  ...
  const [form, setForm] = useState({ title: '', category: '', likelihood: 3, impact: 3 });
  useEffect(() => { load(); }, []);
  async function load() { setRisks(unwrap(await grcApi.listRisks())); const h = await grcApi.heatMap(); setHeat(h.data?.data ?? h.data); }
  async function create(e: React.FormEvent) { e.preventDefault(); try { await grcApi.createRisk(form); setForm({ title: '', category: '', likelihood: 3, impact: 3 }); load(); } catch (err: any) { alert(err.response?.data?.message ?? 'Failed'); } }
  return (
    <div className="space-y-4">
      <form onSubmit={create} className="grid grid-cols-5 gap-2 bg-gray-50 p-3 rounded-lg items-end">
        ...
        <button type="submit" className="bg-indigo-600 text-white px-2 py-1 rounded text-sm">+ Risk</button>
      </form>
      {heat && (
        <div>
      ...
      <table className="w-full text-sm border rounded-lg overflow-hidden">
        <thead className="bg-gray-50"><tr><th className="px-3 py-2 text-left">Risk</th><th className="px-3 py-2 text-right">L</th><th className="px-3 py-2 text-right">I</th><th className="px-3 py-2 text-right">Score</th><th className="px-3 py-2 text-left">Level</th></tr></thead>
        <tbody className="divide-y">
          {risks.length === 0 ? <tr><td colSpan={5} className="px-3 py-6 text-center text-gray-400">No risks.</td></tr> : risks.map((r) => (
            <tr key={r.id}><td className="px-3 py-2">{r.title}</td><td className="px-3 py-2 text-right">{r.likelihood}</td><td className="px-3 py-2 text-right">{r.impact}</td><td className="px-3 py-2 text-right font-bold">{r.score}</td><td className="px-3 py-2"><span className={`text-xs px-1.5 py-0.5 rounded ${SEV[r.level]}`}>{r.level}</span></td></tr>
          ))}
        </tbody>
      </table>
    </div>
