...
| Ph-209/210/212/213 | Contract Lifecycle Management — approved clause library with ordered template assembly (rejects unknown/unapproved), deviation management (flag non-standard text, route to legal, approve/reject), provider-agnostic e-signature envelopes (create→send→sign/decline, stamps contract on completion), 90/60/30-day renewal alerts + initiate-renewal draft | ✅ Done | 11 |
| Ph-211 | Contract obligations/milestones | ⏩ Pre-existing (contracts module: contract-milestone) | — |
...
| Ph-248–250 | Capital Projects & CIP — pjt_capital_configs (capital flag, CIP account, default treatment) + pjt_capital_rules per-task capitalize/expense overrides, cost accumulation classified into CIP vs expense with a CIP summary, and CIP→in-service transfer splitting accumulated capital across multiple assets by percentage | ✅ Done | 8 |

**Running total: 816 tests passing. Track F (Project Portfolio Management) complete.**

### Track G — Platform & Intelligence (delivery log)
...
**Running total: 852 tests passing.**
